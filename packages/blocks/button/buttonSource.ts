...
export const getButtonSource = (authInfo: UseAuthInfoProps) => {
  return `import './button.css';
...
function ButtonImpl({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        '${getOrgName({ authInfo })}' + '-button',
        '${getOrgName({
          authInfo,
        })}' + '-button-variant-' + variant.toLowerCase(),
        '${getOrgName({ authInfo })}' + '-button-size-' + size.toLowerCase(),
        className
      )}
      {...props}
    >
