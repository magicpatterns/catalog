...
export const nameThatColor = ({
  ...
  colorNames.forEach((c) => {
    // If we have an exact match for the color, return it, no processing needed.
    ...
    // Get RGB values from current list item
    const targetRGB = tinycolor({
      r: targetRed,
      g: targetGreen,
      b: targetBlue,
    }).toRgb()

    // Get HSL values from current list item.
    const targetHSL = tinycolor(targetRGB).toHsl()

    // This code is taken from Chirag Mehta (Creative Commons 2.5 License), accessed from https://chir.ag/projects/name-that-color on March 19, 2023
    // This works by taking powers of RGB and HSL, and using these to determine the closest match within the list of colors.  For a deeper explanation, more expertise would be welcome...
    const ndf1 =
      Math.pow(color.r - targetRGB.r, 2) +
      Math.pow(color.g - targetRGB.g, 2) +
      Math.pow(color.b - targetRGB.b, 2)
    const ndf2 =
      Math.pow(h - targetHSL.h, 2) +
      Math.pow(s - targetHSL.s, 2) +
      Math.pow(l - targetHSL.l, 2)
